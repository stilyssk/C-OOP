...
public class Spy
{
    ...
    public string CollectGettersAndSetters(string className)
    {
        ...
        var getterMethod = classMethods.Where(m => m.Name.StartsWith("get")).ToArray();

        //var publicGetMethod = classPublicMethods.Where(m => m.Name.StartsWith("get")).ToArray();
        foreach (MethodInfo currentMethod in getterMethod)
        {
            stringBuilder.AppendLine($"{currentMethod.Name} will return {currentMethod.ReturnType}");
        }

        var setterMethod = classMethods.Where(m => m.Name.StartsWith("set")).ToArray();

        //var nonPublicSetMethod = classNonPublicMethods.Where(m => m.Name.StartsWith("set")).ToArray();
        foreach (MethodInfo currentMethod in setterMethod)
        {
            stringBuilder.AppendLine($"{currentMethod.Name} will set field of {currentMethod.GetParameters().First().ParameterType}");
        }
